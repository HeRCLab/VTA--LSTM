...
#pragma HLS PIPELINE II = 1
        ...
        acc_T ia_tensor[VTA_BATCH][VTA_BLOCK_IN];
        if(insn.inp_or_acc) 
           read_tensor<bus_T, acc_T, acc_idx_T, VTA_BUS_WIDTH, VTA_ACC_WIDTH, VTA_BATCH, VTA_BLOCK_IN>(src_idx, acc_mem, ia_tensor);
        else
        ...
        for (int b = 0; b < VTA_BATCH; b++) {
          for (int oc = 0; oc < VTA_BLOCK_OUT; oc++) {
            ...
            for (int ic = 0; ic < VTA_BLOCK_IN; ic++) {
              ...
              inp_T i_elem = ( insn.inp_or_acc ) ? (inp_T) ia_tensor[b][ic] : i_tensor[b][ic]; 
...
#pragma HLS INLINE
  ...
  VTAAluInsnSI insn = *((VTAAluInsnSI *) &insn_raw);
        ...
        int16_t imm;
        if (uop_offset==0){
          uop = uop_mem[insn.uop_bgn];
          alu_opcode = insn.op_inst1;
          use_imm = insn.use_imm1 ;
          imm     = insn.imm1;
          
        }  
        else if (uop_offset==1)  {
          uop = uop_mem[insn.uop_bgn+1];
          alu_opcode = insn.op_inst2;
          use_imm = insn.use_imm2 ;
          imm     = insn.imm2;
        }  
        else if (uop_offset==2)  {
          uop = uop_mem[insn.uop_bgn+2];
          alu_opcode = insn.op_inst3;
          use_imm = insn.use_imm3;
          imm     = insn.imm3;
        }  
        else {
          uop = uop_mem[insn.uop_bgn+3];
          alu_opcode = insn.op_inst4; 
          use_imm = insn.use_imm4;
          imm     = insn.imm4;
        ...
        // Decode
        acc_idx_T dst_idx =
            uop.range(VTA_UOP_ALU_0_1, VTA_UOP_ALU_0_0) ;
        acc_idx_T src_idx =
            uop.range(VTA_UOP_ALU_1_1, VTA_UOP_ALU_1_0) ;
        ...
        // Perform ALU op over matrix elements
          ...
          #pragma HLS unroll
            // Read in operands
            acc_T src_0 = dst_tensor[0][b];
            acc_T src_1 = (use_imm) ? (acc_T) imm : src_tensor[0][b];
            ...
            if (alu_opcode == VTA_ALU_OPCODE_MIN || alu_opcode == VTA_ALU_OPCODE_MAX) {
              ...
              dst_tensor[0][b] = mix_val;
              o_tensor[0][b] = (out_T) mix_val.range(VTA_OUT_WIDTH - 1, 0);
            } else if (alu_opcode == VTA_ALU_OPCODE_ADD) {
              // Compute Sum
              acc_T add_val =
                  src_0.range(VTA_ACC_WIDTH - 1, 0) + src_1.range(VTA_ACC_WIDTH - 1, 0);
              dst_tensor[0][b] = add_val;
              o_tensor[0][b] = (out_T) add_val.range(VTA_OUT_WIDTH - 1, 0);
            } /*else if (insn.alu_opcode == VTA_ALU_OPCODE_SHR) {
              // Compute Shift Right
              acc_T shr_val = src_0 >> shft_by;
              dst_tensor[0][b] = shr_val;
              o_tensor[0][b] = (out_T) shr_val.range(VTA_OUT_WIDTH - 1, 0);
            } */ else if (alu_opcode == VTA_ALU_OPCODE_MUL) {  
              acc_T mul_val = src_0.range(VTA_MUL_ARG_BIT_WIDTH - 1, 0) * mul_by;
              dst_tensor[0][b] = mul_val;
              o_tensor[0][b] =(out_T) mul_val.range(VTA_OUT_WIDTH - 1, 0);
            
            } else if (alu_opcode == VTA_ALU_OPCODE_MOV) {  
                  dst_tensor[0][b] = src_1;
                  o_tensor[0][b] = (out_T) src_1.range(VTA_OUT_WIDTH - 1, 0);
     ...
     // if (alu_opcode != VTA_ALU_OPCODE_NOP)
     // {
        // Write the results back into accumulator
  ...
  VTAAluInsn insn = *((VTAAluInsn *) &insn_raw);
        ...
        // Perform ALU op over matrix elements
          ...
          #pragma HLS unroll
            // Read in operands
            acc_T src_0 = dst_tensor[0][b];
            acc_T src_1 = insn.use_imm ? (acc_T) insn.imm : src_tensor[0][b];
            aluop_shr_arg_T shft_by = src_1.range(VTA_SHR_ARG_BIT_WIDTH - 1, 0);
            aluop_mul_arg_T mul_by = src_1.range(VTA_MUL_ARG_BIT_WIDTH - 1, 0);
...
#pragma HLS INLINE
  ...
  // Read micro-op fields
  uop_T uop = uop_mem[insn.uop_bgn];


  // Loop offset
  acc_idx_T dst_offset_in = insn.dst_factor_in;
  inp_idx_T src_offset_in = insn.src_factor_in;
  
  acc_idx_T dst_idx=uop.range(VTA_UOP_ALU_0_1, VTA_UOP_ALU_0_0);
  acc_idx_T src_idx=uop.range(VTA_UOP_ALU_1_1, VTA_UOP_ALU_1_0);
  


    // Inner Loop
    EXE_IN_LOOP: for (int it_in = 0; it_in < 4; it_in++) {
    ...
    #pragma HLS unroll
        ...
        #pragma HLS unroll
          ...
          #pragma HLS unroll
            ...
            if (insn.alu_opcode == VTA_ALU_OPCODE_MIN || insn.alu_opcode == VTA_ALU_OPCODE_MAX) {
              ...
              //acc_T mul_val = src_0.range(VTA_ACC_WIDTH - 1, 0) * src_1.range(VTA_ACC_WIDTH - 1, 0);
              acc_T mul_val = src_0.range(VTA_MUL_ARG_BIT_WIDTH - 1, 0) * mul_by;
      ...
      dst_idx += dst_offset_in;
      src_idx += src_offset_in;
    }
...
#pragma HLS DEPENDENCE variable = acc_mem inter false
  ...
  } else if (insn.generic.opcode == VTA_OPCODE_ALU_SINST) {
    SI_ALU: for(int i=0;i<4;i++) {
     //#pragma HLS PIPELINE II=1
     #pragma HLS unroll
       alu_sinst(raw_copy, uop_mem, acc_mem, inp_mem, wgt_mem, out_mem, i); }
  }     
